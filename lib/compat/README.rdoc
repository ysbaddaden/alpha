...
Kokone::Compat is an obstrusive javascript library that
aims at fixing, implementing and emulating standard JavaScript
and DOM in older browsers.
...
Event, before IE8. Kokone::Compat thus emulates Element and
...
don't want to have to think about that, so Kokone::Compat
interfaces the basic methods, like document.getElementById or
...
still relies on attachEvent, detachEvent, etc. Kokone::Compat
thus implements them. Support is limited to bubbling mode only.
The capture mode seems harder to emulate and isn't available
yet (but might be doable). It's also not possible to manually
create and fire events (patches are welcome).

Kokone::Compat also fixes the Event object passed to the callback
...
* manually extend them (eg: kokone.$(elm.parentNode));
...
* you care about IE 6/7? use elm.get(attribute).
...
goal of Kokone::Compat is to implement (or emulate) these
when they're missing. Of course it's slower, but speed issues
can be a good reason to encourage users to upgrade.
...
From what I tested, excanvas.js must be loaded in the document's HEAD,
and any script that needs to access canvas elements must be defered,
or started on load. For instance:
...
If excanvas.js is loaded (before Kokone::Compat that is), created elements
