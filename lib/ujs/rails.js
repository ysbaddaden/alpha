...
(function()
{
  ...
  Alpha.csrf = {
    param: rails_read_meta_content('csrf-param'),
    token: rails_read_meta_content('csrf-token'),
  }
  ...
  function rails_call_with_method(element)
  {
    ...
    if (Alpha.csrf.param && Alpha.csrf.token) {
      html += '<input type="hidden" name="'+ Alpha.csrf.param +'" value="' + Alpha.csrf.token + '"/>';
    }
    form.innerHTML = html;
