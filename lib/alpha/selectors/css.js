...
// It works, but is too slow and has problems.

if (!Element.prototype.querySelectorAll)
{
  Alpha.querySelectorAll = function(cssRules)
  {
    ...
    foundElements = Alpha.extendElements ? Alpha.extendElements(foundElements) : foundElements;
    return new Alpha.NodeList(foundElements);
    ...
    {
      var elements = Alpha.querySelectorAll.operators[operator](parent, cssFilters[0]);
      return Array.prototype.filter.call(elements, function(element)
      {
        for (var i=1, ilen=cssFilters.length; i<ilen; i++)
        {
          if (!Alpha.querySelectorAll.selectors.filter.call(element, cssFilters[i])) {
            return false;
          }
        }
        return true;
...
//          case '~': cssPart.operator = 'sibling';    cssPart.name = parts[++i]; break;
  ...
  Alpha.querySelectorAll.selectors =
  {
    search: function(cssFilter) {
      return Alpha.querySelectorAll.selectors.searches[cssFilter.selector].call(this, cssFilter.name);
    },
    
    filter: function(cssFilter) {
      return Alpha.querySelectorAll.selectors.filters[cssFilter.selector].call(this, cssFilter);
    }
  };
  
  Alpha.querySelectorAll.selectors.searches =
  {
    id: function(id)
    {
    ...
    pseudoSelector: function(pseudoSelector) {
      if (Alpha.querySelectorAll.pseudoSelectors.searches[pseudoSelector]) {
        return Alpha.querySelectorAll.pseudoSelectors.searches[pseudoSelector].call(this);
      }
      throw new Error("Unknown or unsupported CSS pseudo-selector: " + pseudoSelector + ".");
    }
  };
  
  Alpha.querySelectorAll.selectors.filters = {
    ...
    {
      if (Alpha.querySelectorAll.pseudoSelectors.filters[cssFilter.name]) {
        return Alpha.querySelectorAll.pseudoSelectors.filters[cssFilter.name].call(this, cssFilter);
      }
      throw new Error("Unknown or unsupported CSS pseudo-selector: " + cssFilter.name + ".");
    }
  };
  
  Alpha.querySelectorAll.pseudoSelectors = {};
  Alpha.querySelectorAll.pseudoSelectors.searches = {
  ...
  Alpha.querySelectorAll.pseudoSelectors.filters = {
...
//      return (this.get('parentNode').get('lastElementChild') == this);
    ...
    {
      return (Alpha.querySelectorAll.pseudoSelectors.filters['first-child'].call(this)
        && Alpha.querySelectorAll.pseudoSelectors.filters['last-child'].call(this));
    }
  };
  
  Alpha.querySelectorAll.operators =
  {
    descendant: function(parent, cssFilter) {
      return Alpha.querySelectorAll.selectors.search.call(parent, cssFilter);
    },
    
    child: function(parent, cssFilter)
    {
      var elements = Alpha.querySelectorAll.selectors.search.call(parent, cssFilter);
      return Array.prototype.filter.call(elements, function(element) {
    ...
    adjacent: function(previousElement, cssFilter)
    {
      ...
      if (nextElement
        && Alpha.querySelectorAll.selectors.filter.call(nextElement, cssFilter))
      {
        return [nextElement];
      }
      return false;
  ...
  Element.prototype.querySelectorAll = function(cssRule) {
    return Alpha.querySelectorAll.apply(this, arguments);
  }
  
  document.querySelectorAll = function(cssRule) {
    return Alpha.querySelectorAll.apply(document, arguments);
  }
}
