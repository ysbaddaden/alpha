...
It's also not possible to manually create and dispatch events for now, but
this is work in progress (mostly because of Rails' UJS).

Alpha also fixes the Event object passed to the callback --in fact IE doesn't
even pass any Event object. All the elements related to the event are fixed:
