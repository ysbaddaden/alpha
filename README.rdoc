...
uses Mootools, which means I have to understand the jQuery syntax for instance
--which I never understood, and will certainly never. I love frameworks on
servers, but I can't stand JavaScript's. Unless you're building a Rich Internet
Application, but in that case, you'd better switch to something like GWT instead.
...
to have tools that are just JavaScript, and nothing else.

Of course, behing obstrusive, it certainly collapses with frameworks, and I'm
sure there are tons of memory leakages too (feel free to send patches!), but
it works. That's all I'm asking.
...
* UI proposes some user interface widgets, like dialogs, tooltips, autocompleters, date pickers, etc (this one is not obstrusive).

I feel bad about the Addons part, since it goes back to having a library,
but writing Kokone::UI, I felt the necessity to abstract some code out of
it...

You may use Compat or Compat + Addons, or Compat + Addons + UI, or just
parts of these --be sure not to miss any requirements.
